class ras_manager:
    ...
    def generate_skip_token_list(self):
        avg_skip_token_num = int(
            (1 - self.sample_ratio)
            * ((self.height // self.patch_size) // self.vae_size)
            * ((self.weight // self.patch_size) // self.vae_size)
        )
        if self.skip_num_step_length == 0:  # static dropping
            self.skip_token_num_list = [
                avg_skip_token_num for i in range(self.num_steps)
            ]
            ...
            for i in range(self.scheduler_start_step):
                self.skip_token_num_list[i] = 0
            print(
                "Skip token number list: ",
                len(self.skip_token_num_list),
                self.skip_token_num_list,
            )
            return
        for i in range(0, self.num_steps // self.skip_num_step_length + 1):
            for j in range(self.skip_num_step_length):
                ...
                temp_skip_num = avg_skip_token_num + self.skip_num_step * (
                    ...
                    - (
                        (
                            (self.num_steps + self.scheduler_start_step)
                            // self.skip_num_step_length
                        )
                        // 2
                    )
                )
        ...
        for i in range(len(self.skip_token_num_list)):
            assert (
                self.skip_token_num_list[i] >= 0
            ), "Skip token number should be positive"
            assert self.skip_token_num_list[i] <= (
                (self.height // self.patch_size) // self.vae_size
            ) * ((self.weight // self.patch_size) // self.vae_size)
        print(
            "Skip token number list: ",
    ...
    def reset_cache(self):
        ...
        self.current_step = 0
        if (
            self.current_step >= self.scheduler_start_step
            and self.current_step <= self.scheduler_end_step
            and self.current_step not in self.error_reset_steps
        ):
            self.is_RAS_step = True
        else:
            self.is_RAS_step = False
        if (
            self.current_step + 1 >= self.scheduler_start_step
            and self.current_step + 1 <= self.scheduler_end_step
            and self.current_step + 1 not in self.error_reset_steps
        ):
